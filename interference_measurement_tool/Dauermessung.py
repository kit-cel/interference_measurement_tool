...
import GUI_Dauermessung # This file holds our MainWindow and all design related things
...
class ExampleApp(QtWidgets.QMainWindow, GUI_Dauermessung.Ui_MainWindow):
	...
	def Measurement(self): #Durchführung der Messung
		...
		mainPath = uper_path + "/Messungen/Dauermessung/" + Ort
		...
		self.Done.setMaximum(self.NumberOfRepeats.value())
		self.Wait.setMaximum(self.RepetitionRate.value())
		for n in range(1,self.NumberOfRepeats.value()+1):
			self.save_Notefile(mainPath+"/"+str(n),Ort,"Dauermessung")
			Path = mainPath + '/' + str(n)
			...
			self.Done.setValue(n)
			if n != self.NumberOfRepeats.value():
				self.Wait.setValue(0)
				for t in range(1, self.RepetitionRate.value()+1):
					time.sleep(60)
					self.Wait.setValue(self.RepetitionRate.value()-t)
		self.save_mark_pos(mainPath)
		self.Position.clear()
	...
	def edit_CF(self,old_CF,new_CF):
		f = open("IQ-Meas.py",'r')
		filedata = f.read()
		f.close()
		newdata = filedata.replace(old_CF,new_CF)
		f = open("IQ-Meas.py",'w')
		f.write(newdata)
		f.close()
	...
	def change_map(self):		
		if self.Location.currentIndex() == 0:
			...
			self.set_map_value("Südstadt_17_68589_45009.png",68589,45009,17)
